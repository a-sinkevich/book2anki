...
- **Answers should be concise but complete** — typically 1-3 sentences
- **Lists in answers**: when an answer contains a numbered or bulleted list, use <br> between items for readability
- **Bold what the reader had to supply**, with <b>...</b>. THE TEST: cut the bolded words out, and the answer should stop answering the question. That is usually a phrase rather than a single noun, and it is never a word the question already contains — repeating the question's own vocabulary marks nothing. One span for a one-part answer. Where an answer has parallel parts — a list, or the two sides of a contrast — put one span in each part, chosen by the same test, and mark every part or none. Never bold in a question, and where nothing passes the test, use none
